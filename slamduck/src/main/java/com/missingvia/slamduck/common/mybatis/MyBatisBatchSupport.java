 ...
 * http://puras.me/blog/2013/08/22/mybatis-batch-insert-support-both-mysql-and-oracle/
 * 
 * @author Puras.He
...
public class MyBatisBatchSupport {
	...
	private static Logger logger = LoggerFactory.getLogger(MyBatisBatchSupport.class);
    ...
    public static <T> void batchInsertByMapper(SqlSessionFactory sqlSessionFactory, Class<?> mapperClass, Class pojoClass, String methodName, List<?> objList) {
        ...
        try {
        	...
        	logger.error("batchInsertByMapper error:",ex);
            session.rollback();
        } finally {
            session.close();
        }
